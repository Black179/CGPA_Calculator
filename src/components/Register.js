...
const Register = ({ onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      setSuccess('Account created successfully! Welcome to CGPA Calculator!');
      setTimeout(() => {
        // Auto-redirect will happen via AuthContext
      }, 2000); // Show success message briefly
    } else {
      // Provide more specific error messages
      if (result.error === 'User already exists with this email or register number') {
        setError('An account with this email or register number already exists. Please use different credentials or try signing in.');
